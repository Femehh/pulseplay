...
// ─── Audio engine — single AudioContext, fully stop before any new play ────────
function useAudio() {
  ...
  const play = useCallback((freq: number, durationMs: number) => {
    stop(); // always kill any existing tone first

    try {
      // Reuse context if it exists and is running, otherwise create fresh
      ...
      gain.gain.setValueAtTime(0.2, ctx.currentTime + dur - 0.08);
      ...
      osc.onended = () => {
        // auto-cleanup when oscillator finishes naturally
        oscRef.current = null;
        gainRef.current = null;
      };
      oscRef.current = osc;
...
// ─── Utilities ─────────────────────────────────────────────────────────────────
function frequencyScore(target: number, guess: number): number {
...
export default function SoundRecognitionGame({ match, emit, on, solo = false, onExit }: any) {
  ...
  }, [stop, clearTimers, user?.username]);
  ...
  const soloStartRound = useCallback((r: number) => {
    ...
    timerRef.current = setTimeout(() => {
      ...
      setGuessTimeLeft(GUESS_MS / 1000);
      setPhaseSync('guessing');

      let gs = GUESS_MS / 1000;
      intervalRef.current = setInterval(() => {
        gs--;
        setGuessTimeLeft(gs);
        if (gs <= 0) {
  ...
  }, [play, stop, clearTimers, finishSoloRound]);
  ...
  }, [solo]); // intentionally omit soloStartRound to avoid re-running

  // ── Replay tone (guessing phase only, 1.5s burst) ──────────────────────────
  ...
  useEffect(() => {
    ...
    const offShow = on('game:freq_show', (data: any) => {
      ...
      setSubmitted(false);
      setMyRoundScore(null);
      setGuess(440); guessRef.current = 440;
      const ps0 = Math.ceil(data.playDuration / 1000);
      setPlayTimeLeft(ps0);
      setPhaseSync('playing');
      play(data.freq, data.playDuration);
      let ps = ps0;
    ...
    const offHide = on('game:freq_hide', (data: any) => {
      clearTimers(); stop();
      setPhaseSync('guessing');
      let gs = Math.ceil(data.guessDuration / 1000);
      setGuessTimeLeft(gs);
      intervalRef.current = setInterval(() => { gs--; setGuessTimeLeft(gs); if (gs <= 0) clearInterval(intervalRef.current!); }, 1000);
    });
  ...
  }, [submitted, solo, soloSubmit, emit, clearTimers]);
      ...
      <div className="min-h-[calc(100vh-64px)] flex items-center justify-center bg-background p-6">
        ...
        >
          ...
          <div className="text-5xl font-black text-primary my-4">
            {totalSoloScore.current}
            <span className="text-lg text-text-faint">/{maxPossible}</span>
          </div>
      ...
      <div className="bg-surface border-b border-border px-6 py-3">
        <div className="max-w-2xl mx-auto">
          <div className="flex items-center justify-between mb-3">
            ...
            {solo && (
              <button onClick={onExit} className="text-xs text-text-faint hover:text-text underline">
                Exit
              </button>
            )}
          </div>
            ...
            <div className="flex items-center gap-3">
              <div className="flex-1 h-2 bg-surface-3 rounded-full overflow-hidden">
                <motion.div
                  ...
                  animate={{ width: `${(myScore / maxPossible) * 100}%` }}
                />
              </div>
          ...
          >
            ...
            <div className="relative z-10 flex justify-between items-center">
              ...
              <button
                ...
                className="text-xs text-white/40 hover:text-white/80 border border-white/20 hover:border-white/40 rounded-full px-3 py-1 transition-all"
              >
                ▶ replay
              </button>
            ...
            <div className="relative z-10 flex flex-col gap-5">
              <HzSlider value={guess} onChange={(v) => { setGuess(v); guessRef.current = v; }} />
        ...
        {phase === 'result' && targetFreq !== null && (
          ...
          >
            ...
            {solo ? (
              <div className="flex flex-col items-center gap-2">
                <p className="text-text-muted text-sm">
                  Your guess: <span className="text-white font-bold">{submittedGuess}Hz</span>
                </p>
                <div className={`text-6xl font-black ${
                  (myRoundScore ?? 0) >= 80 ? 'text-success'
                  : (myRoundScore ?? 0) >= 50 ? 'text-warning'
                  : 'text-danger'
                }`}>
                  {myRoundScore ?? 0}
                </div>
                ...
                {roundResults.map((r) => (
                  <div key={r.username} className="text-center">
                    ...
                    <div className={`text-4xl font-black ${
                      r.roundScore >= 80 ? 'text-success'
                      : r.roundScore >= 50 ? 'text-warning'
                      : 'text-danger'
                    }`}>+{r.roundScore}</div>
            ...
            <p className="text-text-faint text-sm">
              {round < MAX_ROUNDS ? 'Next round starting...' : 'Finishing...'}
            </p>
          </motion.div>
        )}
      </div>
